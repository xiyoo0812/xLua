...
namespace logger {
    ...
    void log_message::prepare(pchar secbuf, seconds& last) {
        ...
        if (auto now = duration_cast<seconds>(point); now != last) {
            last = now;
            auto time_t = system_clock::to_time_t(time_);
            std::strftime(secbuf, 32, "%Y-%m-%d %H:%M:%S", localtime(&time_t));
        }
    ...
    sstring log_rollingfile<rolling_evaler>::new_log_file_name(const log_time& time) {
        ...
        std::strftime(buffer, sizeof(buffer), "%Y%m%d-%H%M%S", localtime(&time_t));
